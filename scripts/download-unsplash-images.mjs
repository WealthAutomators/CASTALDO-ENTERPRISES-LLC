...
/** Verified downloadable Unsplash IDs (towels, bathroom, beach, spa, home) */
const PHOTO_POOL = [
...
const UNIQUE_POOL = [...new Set(PHOTO_POOL)];
...
function buildTargets() {
  ...
  const sections = [
    { file: "hero/hero-1.jpg", width: 1800 },
    { file: "hero/hero-2.jpg", width: 1800 },
    { file: "hero/hero-3.jpg", width: 1800 },
  ...
  for (const s of sections) targets.push(s);

  for (const product of products) {
    targets.push({ file: `products/${product.slug}-1.jpg`, width: 900, category: product.categorySlug });
    targets.push({ file: `products/${product.slug}-2.jpg`, width: 900, category: product.categorySlug });
  }

  for (let i = 1; i <= 8; i++) {
    targets.push({ file: `reviews/review-${i}.jpg`, width: 600 });
  }
...
async function download(url) {
  const res = await fetch(url, {
    headers: { "User-Agent": "OtanwaCommerce/1.0 (image-setup)" },
    redirect: "follow",
  });
...
async function validatePool() {
  const working = [];
  for (const id of UNIQUE_POOL) {
    try {
      const res = await fetch(unsplashUrl(id, 200), {
        method: "HEAD",
        headers: { "User-Agent": "Mozilla/5.0" },
      });
      if (res.ok) working.push(id);
    } catch {
      /* skip */
    }
  }
...
async function main() {
  ...
  console.log(`Preparing ${targets.length} images...\n`);

  const workingPool = await validatePool();
  console.log(`Validated ${workingPool.length} working Unsplash photos\n`);

  if (workingPool.length < targets.length) {
    throw new Error(`Need ${targets.length} unique photos but only ${workingPool.length} are available`);
  }
  ...
  const getNextPhoto = () => {
    while (poolIndex < workingPool.length) {
      const id = workingPool[poolIndex++];
  ...
  for (const target of targets) {
    ...
    while (!saved && attempts < 25) {
      ...
      const url = unsplashUrl(photoId, target.width);
      ...
      try {
        ...
        if (usedHashes.has(hash)) {
          console.warn(`  skip duplicate content: ${target.file} (${photoId})`);
  ...
  console.log(`\nDone: ${success} downloaded, ${failed} failed, ${usedPhotoIds.size} unique photos used`);
